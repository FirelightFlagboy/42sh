...
/*   Updated: 2018/03/02 22:55:14 by anonymous         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
size_t				binary_directories(t_sh *sh)
{
	...
	in = sh->term->tc_in;
	while (in > 0 && sh->buff[in] != '|' && sh->buff[in] != '`' &&
			sh->buff[in] != '&' && sh->buff[in] != ';')
		in--;
	(in != 0 ? in++ : 0);
	if (!(split = ft_strsplit((sh->buff + in), ' ')))
		return (1);
	val = 0;
	if (split[0] == NULL || (split[0] != NULL &&
				(split[1] == NULL || split[1][0] == '\0') &&
				sh->buff[sh->term->tc_in - 1] != ' '))
		val = 1;
	ft_str2del(&split);
	return (val);
}
...
char				*search_cmd(t_sh *sh)
{
	...
	in = sh->term->tc_in;
	if (in != 0 && (sh->buff[in] == '/' && sh->buff[in - 1] == '.' &&
				(sh->buff[in + 1] == '\0' || sh->buff[in + 1] == ' ')))
		return (ft_strsub(sh->buff, in - 1, in + 1));
	while (in > 0 && sh->buff[in] != '|' && sh->buff[in] != '`' &&
			sh->buff[in] != '&' && sh->buff[in] != ';')
		in--;
	(in != 0 ? in++ : 0);
	if (sh->term->tc_in == 0 || (sh->buff[in] == ' ' &&
				sh->buff[in + 1] == '\0'))
		return (ft_strdup(""));
	i = sh->term->tc_in;
	in = i;
	while (i != 0 && ((((sh->buff)[i] != ' ') &&
					!ft_isquotes(sh->buff[i])) || i == in))
		i--;
	(i != 0 ? i++ : 0);
	while ((sh->buff)[in] != '\0' && (sh->buff)[in] != ' ')
		in++;
	tmp = ft_strsub(sh->buff, i, in - i);
	return (tmp);
}
